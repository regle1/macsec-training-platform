...
execute_silently sudo ip macsec add macsec1 tx sa 0 pn 1 on key 11 11111111111111111111111111111111

# Configure the Receive Channel:
execute_silently sudo ip macsec add macsec1 rx address 0c:99:22:ee:00:00 port 1
execute_silently sudo ip macsec add macsec1 rx address 0c:99:22:ee:00:00 port 1 sa 0 pn 1 on key 22 22222222222222222222222222222222

# Set the IP and bring the interface UP 
execute_silently sudo ifconfig macsec1 10.0.1.1/16
