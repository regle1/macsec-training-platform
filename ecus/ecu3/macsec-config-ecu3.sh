...
execute_silently sudo ip macsec add macsec1 tx sa 0 pn 1 on key 33 33333333333333333333333333333333

# Configure the Receive Channel:
execute_silently sudo ip macsec add macsec1 rx address 0c:42:28:71:00:00 port 1
execute_silently sudo ip macsec add macsec1 rx address 0c:42:28:71:00:00 port 1 sa 0 pn 1 on key 44 44444444444444444444444444444444

# Set the IP and bring the interface UP 
execute_silently sudo ifconfig macsec1 10.0.2.1/16
